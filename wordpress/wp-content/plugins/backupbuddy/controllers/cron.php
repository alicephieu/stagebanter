...
class pb_backupbuddy_cron extends pb_backupbuddy_croncore {
	...
	public function remote_send( $destination, $file, $trigger, $send_importbuddy = false ) {
		pb_backupbuddy::set_greedy_script_limits();
		pb_backupbuddy::status( 'message', 'Sending `' . $file . '` to remote destination `' . $destination . '`. Importbuddy?: `' . $send_importbuddy . '`.' );
	...
	function process_remote_copy( $destination_type, $file, $settings ) {
		...
		if ( $destination_type == 'stash' ) {
			...
			$destination_file = ABSPATH . 'wp-content/uploads/backupbuddy_backups/' . basename( $file );
			if ( file_exists( $destination_file ) ) {
				$destination_file = str_replace( 'backup-', 'backup_copy_' . pb_backupbuddy::random_string( 5 ) . '-', $destination_file );
			}
