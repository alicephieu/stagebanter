...
echo '<div style="float: right; width: 20%; margin-right: 30px;"><br><br>';
	pb_backupbuddy::$ui->start_metabox( 'Things to do . . .', true, true );
		...
		echo '	<li type="disc"><a href="http://ithemes.com/find/plugins/" target="_new">Plugins by iThemes</a></li>';
		echo '	<li type="disc"><a href="http://ithemes.com/find/themes/" target="_new">Themes by iThemes</a></li>';
		echo '</ul>';
	pb_backupbuddy::$ui->end_metabox();
	
	pb_backupbuddy::$ui->start_metabox( 'iThemes News', true, true );
		echo '<p style="font-weight: bold;">iThemes.com</p>';
		echo pb_backupbuddy::$ui->get_feed( 'http://ithemes.com/feed/', 5 );
		echo '<p style="font-weight: bold;">Twitter @backup_buddy</p>';
		
		$twit_append = '<li>&nbsp;</li>';
		$twit_append .= '<li><img src="' . pb_backupbuddy::plugin_url() . '/pluginbuddy/images/twitter.png" style="vertical-align: -3px;" /> <a href="http://twitter.com/pluginbuddy/">Follow @backup_buddy on Twitter.</a></li>';
		$twit_append .= '<li><img src="' . pb_backupbuddy::plugin_url() . '/pluginbuddy/images/twitter.png" style="vertical-align: -3px;" /> <a href="http://twitter.com/ithemes/">Follow @ithemes on Twitter.</a></li>';
		...
		echo pb_backupbuddy::$ui->get_feed( 'https://api.twitter.com/1/statuses/user_timeline.rss?screen_name=backup_buddy', 5, $twit_append, 'Backup_Buddy: ' );
		
	pb_backupbuddy::$ui->end_metabox();
	
	pb_backupbuddy::$ui->start_metabox( 'Help & Support', true, true );
		echo '<p>See our <a href="http://ithemes.com/codex/" target="_new">Knowledge Base Codex</a> or visit our <a href="http://ithemes.com/support/" target="_new">support forum</a> for additional information and help.</p>';
	pb_backupbuddy::$ui->end_metabox();
echo '</div>';
