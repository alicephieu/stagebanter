...
class Dropbox_API {
    ...
    public function getToken($email, $password) {
        ...
        $data = $this->oauth->fetch('http://api.dropbox.com0/token', array(
            'email' => $email, 
            'password' => $password
        ),'POST');
    ...
    public function getAccountInfo() {
        ...
        $data = $this->oauth->fetch('http://api.dropbox.com0/account/info');
    ...
    public function createAccount($email, $first_name, $last_name, $password) {
        ...
        $result = $this->oauth->fetch('http://api.dropbox.com0/account',array(
            'email'      => $email,
     ...
     * @param string $root Use this to override the default root path (sandbox/dropbox)  
     * @return bool/array			true on success, array of results otherwise.
     *								Array
	 *									(
	 *									    [httpStatus] => 200
	 *									    [body] => {"result": "winner!"}
    ...
    public function putFile($path, $file, $root = null) {
        ...
        $result = $this->multipartFetch('http://api-content.dropbox.com/0/files/' . $root . '/' . trim($directory,'/'), $file, $filename);
    ...
    public function copy($from, $to, $root = null) {
        ...
        if (is_null($root)) $root = $this->root;
        $response = $this->oauth->fetch('http://api.dropbox.com0/fileops/copy', array('from_path' => $from, 'to_path' => $to, 'root' => $root));
    ...
    public function createFolder($path, $root = null) {
        ...
        $response = $this->oauth->fetch('http://api.dropbox.com0/fileops/create_folder', array('path' => $path, 'root' => $root),'POST');
        return json_decode($response['body'],true);
    ...
    public function delete($path, $root = null) {
        ...
        if (is_null($root)) $root = $this->root;
        $response = $this->oauth->fetch('http://api.dropbox.com0/fileops/delete', array('path' => $path, 'root' => $root));
    ...
    public function move($from, $to, $root = null) {
        ...
        if (is_null($root)) $root = $this->root;
        $response = $this->oauth->fetch('http://api.dropbox.com0/fileops/move', array('from_path' => $from, 'to_path' => $to, 'root' => $root));
    ...
    public function getLinks($path, $root = null) {
        ...
        $response = $this->oauth->fetch('http://api.dropbox.com0/links/' . $root . '/' . ltrim($path,'/'));
    ...
    public function getMetaData($path, $list = true, $hash = null, $fileLimit = null, $root = null) {
        ...
        $response = $this->oauth->fetch('http://api.dropbox.com0/metadata/' . $root . '/' . ltrim($path,'/'), $args);
